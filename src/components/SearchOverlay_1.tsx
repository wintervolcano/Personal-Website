...
function ensureMinPulsars(base: Pulsar[], minCount: number): Pulsar[] {
  ...
  const out: Pulsar[] = [...base];
  const baseIds = new Set<string>(base.map((p) => (p as any).id as string).filter(Boolean));
  ...
  for (let i = out.length; i < minCount; i++) {
    ...
    const p: any = {
      id,
      name: `Synthetic PSR ${i + 1}`,
      f0_hz: Number(f0.toFixed(3)),
      period_ms: Number((1000 / f0).toFixed(3)),
      difficulty,
    };

    out.push(p as Pulsar);
    baseIds.add(id);
  }
  return out;
}
...
export function SearchOverlay({
  ...
  const allowedCellIndices = useMemo(() => {
    // Avoid top row (nav) and bottom row (overlay/footer), keep middle rows.
    const idxs: number[] = [];
  ...
  const pulsarById = useMemo(() => {
    ...
    for (const p of ALL_PULSARS) m.set((p as any).id as string, p);
    return m;
  }, [ALL_PULSARS]);
  ...
  useEffect(() => {
    ...
    const isNoLockTarget = (el: HTMLElement | null) => {
      ...
      if (el.closest("a,button,[role='button'],input,textarea,select,label")) return true;
      return false;
    };
    ...
    const onClickCapture = (e: MouseEvent) => {
      ...
      scanRef.current = { mouseX: mx, mouseY: my, cellX, cellY, cellIndex, nonce: s0.nonce + 1 };
  ...
  const buildSignal = (s: ScanState, frozenSeed: number | null) => {
    ...
    if (!fft) {
      ...
      const ampDrift = 0.85 + 0.15 * Math.sin((performance.now() / 2400) * 2 * Math.PI);
          ...
          <AnimatePresence>
            {toast ? (
              <motion.div
                ...
                className="inline-flex rounded-full border border-white/14 bg-black/85 backdrop-blur px-4 py-2 text-xs font-semibold tracking-[0.12em] uppercase text-white/80"
              >
                {toast.msg}
              </motion.div>
            ) : null}
                  ...
                  {displayLog.length === 0 ? (
                    ...
                    <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                      {displayLog.map((e, idx) => {
                        const p = pulsarById.get(e.id) ?? BASE_PULSARS.find((x) => ((x as any).id as string) === e.id);
                        if (!p) return null;
